...
OUT_PATH = './output data/Environmental impact and metal recovery results under BS scenario.xlsx'
...
def run_baseline():
    ...
    for _, r in df.iterrows():
        ...
        for m in applicable:
            ...
            for imp in IMPACTS:
                u = L_BASE.get((prov, m, bt, imp), 0.0)
...
print('\nCalculating baseline scenario...')
out = run_baseline()

# Define output column order
meta_cols = ['Scenario', 'Year', 'Province', 'City']
...
# Save results
out.to_excel(OUT_PATH, sheet_name='Baseline scenario', index=False)
print(f'Baseline scenario calculation completed, total {len(out)} rows of data')
