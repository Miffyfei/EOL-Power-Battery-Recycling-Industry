# -*- coding: utf-8 -*-
"""
Baseline (PEV + CEV) - Revised capacity caliber + BCEV weighted by vehicle type proportion × weight coefficient (only affects BCEV weight)
Input:
...
Output:
  ./output data_prediction/BS_EOL power battery from PEV.xlsx
  ./output data_prediction/BS_EOL power battery from CEV.xlsx
"""
...
MAX_REPLACEMENTS = 1  # Modified to 1, indicating only one battery replacement
...
def weibull_scale_from_mean(mu: float, shape: float) -> float:
    # For Weibull(k, λ), mean μ = λ * Γ(1 + 1/k)
    return mu / gamma_fn(1.0 + 1.0/shape)

def annual_interval_probs(shape: float, scale: float, max_age: int):
    """
    Returns:
      P[a-1]   : Probability of event occurring in year a (age interval (a-1, a]), a=1..max_age
      S_end[a-1]: Survival probability at the end of age a S(a) = 1 - F(a)
      F_edge   : F(0..max_age)
    """
...
def renewal_replacement_probs(p_b: np.ndarray, S_end_v: np.ndarray, max_replacements: int):
    ...
    Rk[a-1]: The k-th replacement occurs at age a and the vehicle survives until the end of a
      R1[a-1] = p_b[a-1] * S_v_end[a-1]
      Rk[a-1] = (R_{k-1} * p_b)[a-1] * S_v_end[a-1]
    """
    A = len(p_b)
    R_list = []
    R1 = p_b * S_end_v
    R_list.append(R1.copy())
    # Since MAX_REPLACEMENTS=1, multiple replacement calculations won't be executed here
    # prev = R1
...
# =========================
# 1) Passenger Vehicle (PEV) Parameters - Keep consistent with what you provided
# =========================
...
PEV_BATT_E_KWH = {
    'BPEV_LFP':50*random.uniform(0.6,0.8),'BPEV_NCM111':42*random.uniform(0.6,0.8),'BPEV_NCM523':40.5*random.uniform(0.6,0.8),'BPEV_NCM622':54*random.uniform(0.6,0.8),
    'BPEV_NCM811':78*random.uniform(0.6,0.8),'BPEV_NCA':75*random.uniform(0.6,0.8),'HPEV_LFP':15*random.uniform(0.6,0.8),'HPEV_NCM111':20*random.uniform(0.6,0.8),
    'HPEV_NCM523':18*random.uniform(0.6,0.8),'HPEV_NCM622':35*random.uniform(0.6,0.8),'HPEV_NCM811':40*random.uniform(0.6,0.8),'HPEV_NCA':15*random.uniform(0.6,0.8),
}
...
# =========================
# 2) Commercial Vehicle (CEV) Parameters - Keep consistent with what you provided
# =========================
...
# =========================
# 3) ★ Only for BCEV weight: Vehicle type proportion + weight coefficient (PEV not used)
# =========================
BCEV_TYPE_SHARE = {"Light truck":0.7,"Medium truck":0.1,"Heavy truck":0.1,"Bus/large passenger":0.1}
# Let bus/large passenger reflect 10-ton class weight (with ~800 kg as baseline × 12.5 ≈ 10t)
BCEV_TYPE_WEIGHT_FACTOR = {
    "Light truck": 1.00,
    "Medium truck": 1.30,
    "Heavy truck": 2.50,
    "Bus/large passenger": 12.50
}

def adjusted_bcev_weight(base_weight: float) -> float:
    """BCEV base weight weighted by 'vehicle type proportion × weight coefficient'"""
    s = 0.0
    for t, share in BCEV_TYPE_SHARE.items():
        factor = BCEV_TYPE_WEIGHT_FACTOR.get(t, 1.0)
        s += base_weight * share * factor
    return s

# =========================
# 4) Universal Calculator (Revised capacity caliber + Maximum one replacement)
#    Added weight_adjuster callback for CEV-BCEV scenario; PEV uses identity
# =========================
def compute_city_year_eol_dynamic_params(
    ...
    weight_adjuster=None                   # (model_name:str, base_weight:float) -> adjusted_weight
):
    rows = []
    ...
    for city in df_sales['City'].unique():
        ...
        for y_s in sorted(sub['Year'].unique()):
            ...
            for model in all_models:
                ...
                # Replacement (only one replacement)
                R_list = renewal_replacement_probs(Pb, Svend_v, max_replacements)
                # Since MAX_REPLACEMENTS=1, R_list only contains data for one replacement
                Rsum = R_list[0] if len(R_list) > 0 else np.zeros_like(Pb)

                # Construct "last replacement = r years" survival term with vehicle scrapping (for capacity caliber)
                A = max_age
                ...
                for a in range(1, max_age+1):
                    ...
                    # -- Weight (current year caliber) --
                    _, wt_year_dict = get_params_for_year(y)
                    base_weight = wt_year_dict[model]
                    if weight_adjuster is not None:
                        model_wt_year = weight_adjuster(model, base_weight)
                    else:
                        model_wt_year = base_weight

                    # Expected quantity
                    count_repl = Nb * Rsum[a-1]    # Replacement quantity this year (only one replacement)
                    count_eol  = Nb * Pv[a-1]      # Vehicle scrapping quantity (original or replacement)
                    ...
                    cap_sale = cap_sale_dict[model]                 # Original capacity, based on installation year
                    cap_year_dict, _ = get_params_for_year(y)
                    cap_year = cap_year_dict[model]                 # Replacement capacity this year

                    # Original with vehicle (original battery survives until vehicle scrapping)
                    count_eol_orig = Nb * Pv[a-1] * (1.0 - Fb[a-1])
                    e_gwh_orig = count_eol_orig * cap_sale / 1e6

                    # Replacement this year (current year capacity)
                    e_gwh_repl = (Nb * Rsum[a-1]) * cap_year / 1e6

                    # Replacement with vehicle (replacement year capacity for last replacement)
                    e_gwh_repl_eol = 0.0
                    for r in range(1, a):
                        cap_at_r_dict, _ = get_params_for_year(int(y_s + r))
                        cap_at_r = cap_at_r_dict[model]
...
# =========================
# 5) Weight Adjustment Callbacks
# =========================
def weight_adjuster_pev(model: str, base_weight: float) -> float:
    # PEV: No adjustment
    return base_weight

def weight_adjuster_cev(model: str, base_weight: float) -> float:
    # Only BCEV weighted by vehicle type proportion × coefficient; HCEV unchanged
    if model.startswith("BCEV"):
        return adjusted_bcev_weight(base_weight)
    return base_weight
...
if __name__ == "__main__":
    ...
    # Output paths
    out_pev = './output data/results_EOL power battery/BS_EOL power battery from PEV.xlsx'
    out_cev = './output data/results_EOL power battery/BS_EOL power battery from CEV.xlsx'
    ...
    result_pev = compute_city_year_eol_dynamic_params(
        ...
        weight_adjuster=weight_adjuster_pev
    )

    # Add Province and Code to PEV results
    result_pev = result_pev.merge(pev_city_info, on='City', how='left')
    # Adjust column order
    cols = [col for col in result_pev.columns if col not in ['Province', 'Code']]
    ...
    result_cev = compute_city_year_eol_dynamic_params(
        ...
        weight_adjuster=weight_adjuster_cev
    )
