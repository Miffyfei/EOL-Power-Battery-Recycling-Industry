# -*- coding: utf-8 -*-
"""
Energy Density (ED) Scenario + CEV Vehicle Type Proportion × Weight Coefficient Weighting (Only affects BCEV weight)
- Exactly the same as Baseline, only difference: get_battery_parameters(year)
  * year<=2024: Returns baseline's original single-vehicle electricity/weight
  * year>=2025: Adjusts battery capacity and weight based on 3.6% annual energy density growth
- Capacity caliber: Installation year capacity (already revised)
- Output:
  ./output data_prediction/ED_EOL power battery from PEV.xlsx
  ./output data_prediction/ED_EOL power battery from CEV.xlsx
"""
...
def annual_interval_probs(shape: float, scale: float, max_age: int):
    ...
    F = weibull_min.cdf(ages, shape, scale=scale)
    P = F[1:] - F[:-1]
    S_end = 1.0 - F[1:]
    return P, S_end, F

def renewal_replacement_probs(p_b: np.ndarray, S_end_v: np.arange, max_replacements: int):
    A = len(p_b)
    R_list = []
    R1 = p_b * S_end_v
...
# =========================
# 1) Passenger Vehicle/Commercial Vehicle Parameters (Key parameters unchanged)
# =========================
...
# Base year (2024) battery weight and energy density
PEV_BATT_WEIGHT_KG_2024 = {
...
PEV_BATT_ENERGY_DENSITY_WH_KG_2024 = {
    "BPEV_LFP": 143, "BPEV_NCM111": 120, "BPEV_NCM523": 134, "BPEV_NCM622": 177,
    "BPEV_NCM811": 163, "BPEV_NCA": 361, "HPEV_LFP": 42, "HPEV_NCM111": 60,
    "HPEV_NCM523": 65, "HPEV_NCM622": 140, "HPEV_NCM811": 176, "HPEV_NCA": 54
}
...
CEV_BATT_ENERGY_DENSITY_WH_KG_2024 = {
    "BCEV_LFP": 190, "BCEV_NCM111": 210, "BCEV_NCM523": 230, "BCEV_NCM622": 216,
    "BCEV_NCM811": 236, "BCEV_NCA": 237, "HCEV_LFP": 188, "HCEV_NCM111": 206,
    "HCEV_NCM523": 230, "HCEV_NCM622": 255, "HCEV_NCM811": 289, "HCEV_NCA": 314
}

def get_energy_density_factor(year:int) -> float:
    # 3.6% annual improvement starting from 2025
    return 1.0 if year <= 2024 else (1.0 * (1.036 ** (year - 2024)))

def get_battery_parameters(year:int, is_pev: bool):
    # year<=2024: Returns "original estimated capacity/weight"; otherwise adjusted by ED factor
    battery_weights_2024 = PEV_BATT_WEIGHT_KG_2024 if is_pev else CEV_BATT_WEIGHT_KG_2024
    energy_density_2024 = PEV_BATT_ENERGY_DENSITY_WH_KG_2024 if is_pev else CEV_BATT_ENERGY_DENSITY_WH_KG_2024

    if year <= 2024:
        # Original capacity calculation: energy density(wh/kg) * weight(kg) / 1000 = capacity(kwh)
        battery_capacity = {k: v * battery_weights_2024[k] / 1000.0 for k, v in energy_density_2024.items()}
        return battery_capacity, battery_weights_2024
    else:
        factor = get_energy_density_factor(year)
        models = list(PEV_SHARE.columns) if is_pev else list(CEV_SHARE.columns)

        # Adjust battery parameters based on energy density growth
        adjusted_energy_density = {m: energy_density_2024[m] * factor for m in models}

        # Weight adjusted by random factor (becomes lighter each year)
        adjusted_battery_weights = {m: battery_weights_2024[m] * random.uniform(0.85, 1) for m in models}

        # Capacity = energy density(wh/kg) * weight(kg) / 1000 = capacity(kwh)
        adjusted_battery_capacity = {m: adjusted_energy_density[m] * adjusted_battery_weights[m] / 1000.0 for m in models}

        return adjusted_battery_capacity, adjusted_battery_weights
...
# =========================
# 2) Revised Capacity Caliber Calculator (Same as Baseline; BCEV weight weighted)
# =========================
def compute_city_year_eol_dynamic_params(
    ...
    is_pev: bool,
    out_path: str,
    get_params_for_year,                   # Function (year)->(cap_dict, wt_dict)
    ...
    for city in df_sales['City'].unique():
        ...
        for y_s in sorted(sub['Year'].unique()):
            ...
            # Vehicle lifespan distribution
            mu_v = vehicle_mean_life_by_sale_year(int(y_s), is_pev=is_pev)
            lam_v = weibull_scale_from_mean(mu_v, VEHICLE_SHAPE_DEFAULT)
            Pv, Svend_v, Fv_edge = annual_interval_probs(VEHICLE_SHAPE_DEFAULT, lam_v, max_age)
            ...
            for model in all_models:
                ...
                # Sales year capacity (original installation year)
                cap_sale_dict, wt_sale_dict = get_params_for_year(int(y_s))
                cap_sale = cap_sale_dict[model]

                # Construct probability of "last replacement in year r" flowing to scrapping in year a
                A = max_age
...
if __name__ == "__main__":
    ...
    out_pev = './output data/results_EOL power battery/ED_EOL power battery from PEV.xlsx'
    out_cev = './output data/results_EOL power battery/ED_EOL power battery from CEV.xlsx'
    ...
    result_pev = compute_city_year_eol_dynamic_params(
        ...
        out_path=None, get_params_for_year=lambda y: get_battery_parameters(y, True)
    )
    ...
    result_cev = compute_city_year_eol_dynamic_params(
        ...
        out_path=None, get_params_for_year=lambda y: get_battery_parameters(y, False)
    )
